...
class Hangman(commands.Cog):
    ...
    async def hangman(self, ctx, *, member: discord.Member = None):
        ...
        embed = self.hangman_embed("Welcome to Hangman! "
        ...
        if self.lives == 0:
            await ctx.send(content=f"Unfortunately, you died at {len(self.guesses)} guesses. "
                                   f"The word was {self.word}.")
        else:
            await ctx.send(content=f"Congratulations! "
